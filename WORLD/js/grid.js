class Grid {
  ...
  static load(canvas, world, info) {
    ...
    for (let i = 0; i < grid.height / grid.cellSize; i++) {
      ...
      for (let j = 0; j < grid.width / grid.cellSize; j++) {
        ...
        grid.cells[i][j].worldItemIndices = info.cells[i][j].worldItemIndices;
      }
    }
  ...
  getDataFromNearbyCells(loc) {
    ...
    this.cells[y - 1] && this.cells[y - 1][x]
      ? cells.push(this.cells[y - 1][x])
      : true;
    this.cells[y + 1] && this.cells[y + 1][x]
      ? cells.push(this.cells[y + 1][x])
      : true;
    this.cells[y - 1] && this.cells[y - 1][x - 1]
      ? cells.push(this.cells[y - 1][x - 1])
      : true;
    this.cells[y] && this.cells[y][x - 1]
      ? cells.push(this.cells[y][x - 1])
      : true;
    this.cells[y + 1] && this.cells[y + 1][x - 1]
      ? cells.push(this.cells[y + 1][x - 1])
      : true;
  ...
  getCellsInActiveRegion(activeRegion) {
    ...
    for (const row of this.cells) {
      for (const cell of row) {
        if (
          activeRegion.containsPoly(cell.poly) ||
          cell.poly.containsPoly(activeRegion)
        ) {
          cells.push(cell);
        }
      }
  ...
  generateRoadItemsFromIndices(world) {
    for (const row of this.cells) {
      for (const cell of row) {
        ...
        cell.worldItems.graphSegments = cell.worldItemIndices.graphSegments.map(
